...
struct IntervalEditorView: View {
    ...
    func moveIntervals(from source: IndexSet, to destination: Int) {
        ...
        // Update the order property for each interval after moving
...
struct IntervalDetailView: View {
    ...
    init(
        interval: TimerInterval, isNew: Bool, onSave: @escaping (TimerInterval) -> Void,
                        ...
                        ForEach(colors, id: \.self) { colorHex in
                            Circle()
                                ...
                                .overlay(
                                    Circle()
                                        .stroke(
                                            colorHex == color ? Color.white : Color.clear,
                                            lineWidth: 2
                                        )
                                        .padding(2)
                                )
                                .overlay(
                                    Circle()
                                        .stroke(
                                            colorHex == color ? Color.black : Color.clear,
                                            lineWidth: 1
                                        )
                                        .padding(3)
                                )
